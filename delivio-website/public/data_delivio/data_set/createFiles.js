...
function createFiles(callback, filename = "output.txt", dirName="/data_delivio/good_photos") {
...
createFiles((dirName, data) => {
	...
	for(const entry in new_data_arr) {
		map.set(new_data_arr[entry].replace(".png", ""), result[entry]);
	}
	...
	fs.readFile("./data_set/final.json", (err, data) => {
		...
		for(const entry of res_json) {
			entry.image = map.get(entry.id) == null ? "default" : map.get(entry.id)
		}
